...
- 通过上面三道题，我们可以总结出滑动窗口算法的抽象思想：
  - right指针元素依次入队列，扩大滑动窗口大小，直到满足条件为止
  - 然后右移left指针，缩小滑动窗口大小找出使得条件不成立的left指针
  - 求解结果
...
- 第二种滑动窗口算法思路：使用双指针配合辅助数据结构（可以是map、set、list、vector或者是单调队列）实现
  - right指针向右扩展遍历每个元素，判断每个入队列的元素是否满足条件
  - 如果满足条件，则right继续扩展，并更新结果
  - 如果不满足条件，则当前滑动窗口需要进行汇总计算，并更新left指针的大小

- 第三种滑窗算法思路：直接使用底层数据结构vector、单调队列、list等，将元素依次入队列，并判断条件和滑窗大小
